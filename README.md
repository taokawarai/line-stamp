...
```
以下のようなLINEのスタンプのアイデアを96種類考えてください。ワードとセットでキャラクターのポーズなどのイメージもあればかっこの中に記述してください。

【特集参加条件】
・新規のスタンプ・絵文字であること（販売済みのスタンプ・絵文字のアップデート申請はNG）
・パッケージ内の1個以上に "一年中使えるような挨拶や返答等の表現" が含まれていること
　例：おはよう／おつかれ／おやすみ／了解／ありがとう／ごめん／よろしく／おめでとう など
・キャラクターや表現方法は自由
　※内容がテーマにそぐわないと判断させていただいた場合はリジェクトさせていただきます。

【推奨】
・春夏秋冬それぞれの季節やイベント毎に使えるような挨拶やお祝い等の表現もあわせて含めること
　例：暑中お見舞い申し上げます／メリークリスマス／お誕生日おめでとう／ハッピーハロウィン／良いお年を／入学・卒業おめでとう など

【NG事項】
・名前などを含む特定個人向けのスタンプ・絵文字
・スタンプ・絵文字の大量申請行為

形式は単語を12個ずつ以下のフォーマットで出力してください。
...
```
target_user: "日本の野球ファン"
layout:
  rows: 3
  cols: 4
  mode: “grid / sprite-sheet”
  no_cell_borders: true
background:
  chroma_key_color_hex: "#9ACD32"
character:
  # ...（省略）...
...
notes:
  - "背景は必ず #9ACD32 単色"
  - "12 セルすべて異なる構図"
  - "words の文言を 1 セルにつき 1 つ表示"
  - "words は日本語"
  - "文言の意味に合うポーズを選択"
  - "白縁文字が欠けないよう調整"
  - "no borders, no grid lines, no cell separators"
